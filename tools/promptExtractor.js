...
const NEW_PROMPT_ASSIGNMENTS = [
  ...
  // 2.1.187 — 57 new/fuzzy-miss prompts from the 2.1.185→2.1.187 bump.
  // Fuzzy-misses: content still present in binary but opening shifted past the
  // 100-char fingerprint, so the fuzzy-carryover pass dropped the name.
  {
    matcher: t => t.includes('resources/directory/read') && t.includes('Parameters:'),
    name: 'Tool Description: Read MCP resource (long form)',
    id: 'tool-description-read-mcp-resource',
    description: 'MCP resource directory listing tool — long form with full parameter docs and resources/directory/read annotation',
  },
  {
    matcher: t => t.includes('List the direct children of a directory resource on an MCP server.') && !t.includes('resources/directory/read'),
    name: 'Tool Description: Read MCP resource (short form)',
    id: 'tool-description-read-mcp-resource-short',
    description: 'MCP resource directory listing tool — concise variant without resources/directory/read annotation',
  },
  {
    matcher: t => t === 'resources/directory/read',
    name: 'Data: MCP resource directory-read label',
    id: 'data-mcp-resource-dir-read-label',
    description: 'Bare label string for the resources/directory/read MCP resource type — used in UI and error messages',
  },
  {
    matcher: t => t.includes('returned InvalidParams on cursor'),
    name: 'Data: MCP resource directory-read cursor error',
    id: 'data-mcp-resource-dir-read-cursor-error',
    description: 'Error message when a paginated directory read returns InvalidParams on the cursor — returns entries from prior pages',
  },
  {
    matcher: t => t.includes('stopped at') && t.includes('pages with more pending'),
    name: 'Data: MCP resource directory-read truncated',
    id: 'data-mcp-resource-dir-read-truncated',
    description: 'Truncation notice when a directory read stops early with more pages pending',
  },
  {
    matcher: t => t.includes('not a directory') && t.includes('resources/directory/read returned'),
    name: 'Data: MCP resource directory-read not-a-directory error',
    id: 'data-mcp-resource-dir-read-not-dir-error',
    description: 'Error when the resource URI passed to resources/directory/read is not a directory',
  },
  {
    matcher: t => t.includes('is temporarily unavailable') && t.includes('Wait briefly and then try'),
    name: 'Data: Auto-mode safety classifier unavailable',
    id: 'data-auto-mode-safety-classifier-unavailable',
    description: 'Error message shown when the auto-mode safety classifier is temporarily unavailable — instructs user to wait and retry',
  },
  {
    matcher: t => t.includes("summary: '<5-10 word recap>'") && t.includes('agentId') && !t.includes('Async agent launched'),
    name: 'System Prompt: Subagent ID usage footer',
    id: 'system-prompt-subagent-id-usage-footer',
    description: 'Footer injected into subagent responses giving the agentId and SendMessage continuation instructions',
  },
  {
    matcher: t => t.includes('Async agent launched successfully') && t.includes('agentId') && t.includes('internal ID'),
    name: 'System Prompt: Async agent launched result',
    id: 'system-prompt-async-agent-launched-result',
    description: 'Message returned when an async agent is launched — provides agentId and SendMessage instructions for the parent',
  },
  {
    matcher: t => t.includes('.label') && t.includes('Compact it to under') && t.includes('.targetDesc'),
    name: 'System Reminder: Memory index over budget compaction',
    id: 'system-reminder-memory-index-over-budget-compaction',
    description: 'Reminder injected when a memory file exceeds its size budget — instructs the model to compact it in place',
  },
  {
    matcher: t => t.includes('You MUST call the') && t.includes('tool to complete this request'),
    name: 'System Reminder: Stop call required tool',
    id: 'system-reminder-stop-call-required-tool',
    description: 'Reminder injected when a structured-output or stop tool call is required — demands the model call the named tool immediately',
  },
  {
    matcher: t => t.includes('The user selected the lines') && t.includes('.lineStart'),
    name: 'System Reminder: Lines selected in IDE',
    id: 'system-reminder-lines-selected-in-ide',
    description: 'Reminder injected when the user has selected lines in the IDE — provides the selected content and notes it may or may not be relevant',
  },
  {
    matcher: t => t.includes('Review target: GitHub pull request') && t.includes('gh pr'),
    name: 'Agent Prompt: Review PR slash command',
    id: 'agent-prompt-review-pr-slash-command',
    description: 'System prompt injected for /review PR — instructs the agent to gather the PR diff via gh CLI and run the code review skill',
  },
  // Genuinely new in 2.1.187:
  {
    matcher: t => t.includes('.hookName') && t.includes('.message') && t.includes('hook stopped continuation'),
    name: 'System Reminder: Hook stopped continuation',
    id: 'system-reminder-hook-stopped-continuation',
    description: 'Template message when a hook stops continuation — includes hookName and message interpolations',
  },
  {
    matcher: t => t.trim() === 'hook stopped continuation:',
    name: 'System Reminder: Hook stopped continuation (bare label)',
    id: 'system-reminder-hook-stopped-continuation-label',
    description: 'Bare label prefix for hook-stopped continuation messages without template interpolations',
  },
  {
    matcher: t => t.includes('EDIT REMOVALS: Edit calls show both') && t.includes('removes'),
    name: 'Agent Prompt: Code review edit removals guidance',
    id: 'agent-prompt-code-review-edit-removals',
    description: 'Code-review context fragment — instructs reviewers to judge deletions (removes) as seriously as additions',
  },
  {
    matcher: t => t.includes('Your local copy may be out of date'),
    name: 'System Reminder: Plugin out of date',
    id: 'system-reminder-plugin-out-of-date',
    description: 'Warning appended to plugin name when the local plugin copy may be stale — prompts update via CLI or Marketplaces',
  },
  {
    matcher: t => t.includes('authenticates automatically with your Claude login'),
    name: 'System Reminder: MCP auto-auth with Claude login',
    id: 'system-reminder-mcp-auto-auth',
    description: 'MCP server status message — server authenticates automatically; instructs claude login if not signed in',
  },
  {
    matcher: t => t.includes('is from .mcp.json and awaiting approval'),
    name: 'System Reminder: MCP server awaiting approval',
    id: 'system-reminder-mcp-awaiting-approval',
    description: 'MCP server status message — server from .mcp.json is awaiting project-level approval; instructs user to run claude in the directory',
  },
  {
    matcher: t => t.includes('is from .mcp.json and was rejected'),
    name: 'System Reminder: MCP server rejected',
    id: 'system-reminder-mcp-rejected',
    description: 'MCP server status message — server from .mcp.json was rejected; instructs mcp reset-project-choices to review again',
  },
  {
    matcher: t => t.includes('# Claude Code agent proxy') && t.includes('Outbound HTTPS from this session'),
    name: 'System Reminder: Agent proxy details',
    id: 'system-reminder-agent-proxy-details',
    description: 'Full agent-proxy info block injected as a system reminder — explains the local proxy URL and CA bundle for outbound HTTPS',
  },
  {
    matcher: t => t.includes('The model for this session has been changed to') && t.includes('system-reminder'),
    name: 'System Reminder: Model changed',
    id: 'system-reminder-model-changed',
    description: 'Reminder injected when the session model is switched mid-conversation — announces new model name and running identity',
  },
  {
    matcher: t => t.includes('credentials block was dropped'),
    name: 'System Reminder: Credential validation error',
    id: 'system-reminder-credential-validation-error',
    description: 'Error injected when a credentials block fails schema validation and is dropped — no credential protection applied until fixed',
  },
  {
    matcher: t => t.includes('safeguards flagged this message. The safeguards are intentionally broad'),
    name: 'System Reminder: Safeguards flagged message',
    id: 'system-reminder-safeguards-flagged',
    description: 'Injected when CC safeguards flag a message — explains broad heuristics and links to the safety docs',
  },
  {
    matcher: t => t.includes('[_t.kind]} · ${.description}'),
    name: 'Data: Tool kind description format',
    id: 'data-tool-kind-description-format',
    description: 'Short format string combining a tool kind label with its description — used in tool-picker UI',
  },
  {
    matcher: t => t.includes('in Model Garden to upgrade'),
    name: 'System Reminder: Model fallback (Model Garden)',
    id: 'system-reminder-model-fallback-billing',
    description: 'Model unavailability notice with Model Garden upgrade link — shown when the preferred model is not available in billing context',
  },
  {
    matcher: t => t.includes('in the Bedrock console to upgrade'),
    name: 'System Reminder: Model fallback (Bedrock)',
    id: 'system-reminder-model-fallback-settings',
    description: 'Model unavailability notice with Bedrock console upgrade link — shown when the preferred model is not available in Bedrock context',
  },
  {
    matcher: t => t.includes('safeguards flagged this message for a cybersecurity topic'),
    name: 'System Reminder: Safeguards flagged (cybersecurity)',
    id: 'system-reminder-safeguards-flagged-cybersecurity',
    description: 'Injected when CC safeguards flag a cybersecurity-topic message — explains access and links to policy acknowledgment',
  },
  {
    matcher: t => t.includes('had no active task; resumed from transcript') && t.includes('ran to completion. Result:'),
    name: 'System Reminder: SendMessage resumed (no active task)',
    id: 'system-reminder-send-message-resumed-no-task',
    description: 'SendMessage result when target agent had no active task — resumed from transcript and ran to completion synchronously',
  },
  {
    matcher: t => t.includes('had no active task; resumed from transcript') && t.includes('in the background with your message'),
    name: 'System Reminder: SendMessage resumed background (no active task)',
    id: 'system-reminder-send-message-resumed-no-task-background',
    description: 'SendMessage result when target agent had no active task — resumed in background; notification will arrive on completion',
  },
  {
    matcher: t => t.includes('was stopped') && t.includes('resumed it with your message and ran to completion'),
    name: 'System Reminder: SendMessage resumed (was stopped)',
    id: 'system-reminder-send-message-resumed-stopped',
    description: 'SendMessage result when target agent was stopped — resumed with the message and ran to completion',
  },
  {
    matcher: t => t.includes('the call was held back fail-closed'),
    name: 'Data: Auto-mode security monitor holdback',
    id: 'data-auto-mode-security-monitor-holdback',
    description: 'Fragment of the security monitor continuation prompt explaining fail-closed holdback behavior for ambiguous tool calls',
  },
  {
    matcher: t => t.includes('Authenticated with') && t.includes('Its tools are now available in Claude Code'),
    name: 'System Reminder: MCP authenticated',
    id: 'system-reminder-mcp-authenticated',
    description: 'Confirmation message after successful MCP server authentication — tools now available',
  },
  {
    matcher: t => t.includes('awsAuthRefresh failed'),
    name: 'System Reminder: AWS auth refresh failed',
    id: 'system-reminder-aws-auth-refresh-failed',
    description: 'Error message when the AWS credential refresh command fails — instructs user to check settings and retry in a separate terminal',
  },
  {
    matcher: t => t.includes('Could not add design scopes to the token') && t.includes('Claude account with subscription'),
    name: 'Tool Result: Design scope token failed',
    id: 'tool-result-design-scope-failed',
    description: 'DesignSync error when design scopes cannot be added to the token — instructs /login with Claude account subscription',
  },
  {
    matcher: t => t.includes('Could not add design scopes to the token') && t.includes('interactive Claude Code terminal'),
    name: 'Tool Result: Design scope token failed (non-interactive)',
    id: 'tool-result-design-scope-failed-noninteractive',
    description: 'DesignSync scope error in non-interactive context — re-authentication requires an interactive terminal',
  },
  {
    matcher: t => t.includes('Could not refresh the design access token') && t.includes('if the error persists'),
    name: 'Tool Result: Design token refresh failed',
    id: 'tool-result-design-refresh-failed',
    description: 'DesignSync transient error when the design access token cannot be refreshed — retry or re-authenticate if it persists',
  },
  {
    matcher: t => t.includes('Could not refresh the design access token') && t.includes('or run /design-login to re-authorize'),
    name: 'Tool Result: Design token refresh failed (simple)',
    id: 'tool-result-design-refresh-failed-simple',
    description: 'Shorter design token refresh failure — offers /design-login as a direct re-authorization path',
  },
  {
    matcher: t => t.includes("Couldn't build the claude.ai authorization link"),
    name: 'Tool Result: MCP auth link failed',
    id: 'tool-result-mcp-auth-link-failed',
    description: 'MCP auth error when the claude.ai authorization link cannot be built — prompts user to sign in',
  },
  {
    matcher: t => t.includes("Couldn't turn on usage credits. Run /usage-credits to try again"),
    name: 'Tool Result: Usage credits enable failed',
    id: 'tool-result-usage-credits-enable-failed',
    description: 'Error when usage credits could not be enabled — instructs user to run /usage-credits to retry',
  },
  {
    matcher: t => t.includes("Couldn't turn on usage credits — no charge was made"),
    name: 'Tool Result: Usage credits enable failed (no charge)',
    id: 'tool-result-usage-credits-enable-failed-no-charge',
    description: 'Usage credits enable failure confirming no charge was made — softer variant for cases where billing was not attempted',
  },
  {
    matcher: t => t === "Couldn't turn on usage credits",
    name: 'Tool Result: Usage credits enable failed (bare)',
    id: 'tool-result-usage-credits-enable-failed-bare',
    description: 'Bare usage-credits enable failure string without instructions — used as a UI label or short toast message',
  },
  {
    matcher: t => t.includes('design-login requires an interactive terminal'),
    name: 'Tool Result: DesignSync needs login (non-interactive)',
    id: 'tool-result-design-needs-login-noninteractive',
    description: 'DesignSync non-interactive fallback — design-login is unavailable; instructs user to run /design-login in an interactive terminal',
  },
  {
    matcher: t => t.includes('Dynamic workflows let Claude write a script that orchestrates many agents'),
    name: 'Data: Tip — dynamic workflows',
    id: 'data-tip-dynamic-workflows',
    description: 'Tip surfaced in the cooldown provider — explains dynamic workflows and how to trigger them with the ultracode keyword',
  },
  {
    matcher: t => t.includes('Create the file first (Write tool'),
    name: 'Tool Result: Artifact file not found',
    id: 'tool-result-artifact-file-not-found',
    description: 'ArtifactTool error when the target file does not exist — instructs model to create the file first then retry',
  },
  {
    matcher: t => t.includes('hosts not in allowedDomains are denied without consulting the ask callback'),
    name: 'Data: Schema — deny unlisted hosts',
    id: 'data-schema-deny-unlisted-hosts',
    description: 'Schema .describe() for the denyUnlisted proxy option — when true, hosts not in allowedDomains are denied without the ask callback',
  },
  {
    matcher: t => t.includes('List of denied domains. Unlike allowedDomains'),
    name: 'Data: Schema — denied domains',
    id: 'data-schema-denied-domains',
    description: 'Schema .describe() for the deniedDomains proxy config — accepts bare * as deny-all unlike allowedDomains',
  },
  {
    matcher: t => t.includes('Memory paused for this session'),
    name: 'Tool Result: Memory paused',
    id: 'tool-result-memory-paused',
    description: 'Return value from the pause-memory slash command — confirms memory is paused and previously-loaded memories are still visible',
  },
  {
    matcher: t => t.includes("restricted by your organization's settings. Run /model"),
    name: 'System Reminder: Model restricted by org',
    id: 'system-reminder-model-restricted',
    description: 'Error shown when the requested model is restricted by org settings — instructs user to run /model to choose a different one',
  },
  {
    matcher: t => t.includes('No MCP server named') && t.includes('Did you mean'),
    name: 'Tool Result: MCP server not found',
    id: 'tool-result-mcp-server-not-found',
    description: 'Error when an MCP server name is not recognized — suggests the closest match and lists all via claude mcp list',
  },
  {
    matcher: t => t.includes('Opened') && t.includes('.needsSetup') && t.includes('usage credits'),
    name: 'Tool Result: Usage credits browser opened',
    id: 'tool-result-usage-credits-browser-opened',
    description: 'Confirmation that the usage-credits management page was opened in the browser',
  },
  {
    matcher: t => t.includes('Visit') && t.includes('.needsSetup') && t.includes('usage credits'),
    name: 'Tool Result: Usage credits visit URL',
    id: 'tool-result-usage-credits-visit-url',
    description: 'Usage credits URL without browser-open — instructs user to visit the URL manually to manage credits',
  },
  {
    matcher: t => t.includes('pre-configured agent proxy (CA bundle:'),
    name: 'System Reminder: Agent proxy CA info',
    id: 'system-reminder-agent-proxy-ca-info',
    description: 'Short agent-proxy info injected per turn — lists the CA bundle path and advises on TLS failures or 403s',
  },
  {
    matcher: t => t === 'Pause automemory for this session',
    name: 'Data: Slash command description — pause memory',
    id: 'data-slash-command-pause-memory',
    description: 'Description field for the pause-memory slash command shown in the command picker',
  },
  {
    matcher: t => t.includes('enabled at project scope (.claude/settings.json'),
    name: 'System Reminder: Plugin project scope',
    id: 'system-reminder-plugin-project-scope',
    description: 'Warning shown when a plugin disable attempt targets a project-scoped plugin — explains how to disable per-user instead',
  },
  {
    matcher: t => t.includes('Push a React design system to claude.ai/design') && !t.startsWith('---'),
    name: 'Tool Description: Design push',
    id: 'tool-description-design-push',
    description: 'Description for the design-push tool — bundles real component code from Storybook and uploads to claude.ai/design',
  },
  {
    matcher: t => t.includes('Review a GitHub pull request; for your working diff use /code-review'),
    name: 'Tool Description: Review PR slash command',
    id: 'tool-description-review-pr-slash-command',
    description: 'Short description for the /review command shown in the command picker — distinguished from /code-review (working diff)',
  },
  {
    matcher: t => t.includes('Subagent nesting limit reached (depth'),
    name: 'System Reminder: Subagent nesting limit',
    id: 'system-reminder-subagent-nesting-limit',
    description: 'Error injected when Agent tool calls hit the max nesting depth — instructs model to complete the task directly',
  },
  {
    matcher: t => t.includes('The Claude GitHub App is now installed'),
    name: 'Tool Result: GitHub App installed',
    id: 'tool-result-github-app-installed',
    description: 'Post-install confirmation for the Claude GitHub App — optionally offers GitHub Actions workflow setup',
  },
  {
    matcher: t => t.includes('this proxy only accepts HTTPS CONNECT tunnels'),
    name: 'Data: Proxy HTTPS-only error',
    id: 'data-proxy-https-only-error',
    description: 'Error from the agent proxy 405 handler — explains that only HTTPS CONNECT tunnels are accepted, not plain-HTTP',
  },
  {
    matcher: t => t.includes('uncommitted') && t.includes('All will be lost if you remove'),
    name: 'System Reminder: Uncommitted files warning',
    id: 'system-reminder-uncommitted-files-warning',
    description: 'Warning injected before a worktree remove when there are uncommitted files or unpushed commits that would be lost',
  },
  {
    matcher: t => t.includes("You're already connected via the GitHub App"),
    name: 'Tool Result: GitHub App already connected',
    id: 'tool-result-github-app-already-connected',
    description: 'Warning shown when attempting to reconnect the GitHub App — continuing will replace the existing credential',
  },
  {
    matcher: t => t.includes("You're out of usage credits. Run /usage-credits to keep using Fable 5"),
    name: 'Data: Usage credits out (Fable 5)',
    id: 'data-usage-credits-out-fable5',
    description: 'Error message when usage credits are exhausted — offers /usage-credits to top up or /model to switch away from Fable 5',
  },
  {
    matcher: t => t.includes('Ask your admin to raise it to keep using Fable 5'),
    name: 'Data: Usage credits spend cap — admin raise (Fable 5)',
    id: 'data-usage-credits-spend-cap-admin-fable5',
    description: 'Monthly spend cap message for non-self-serve users — instructs them to ask their admin to raise the limit',
  },
  {
    matcher: t => t.includes('Run /usage-credits to manage your limit and keep using Fable 5'),
    name: 'Data: Usage credits spend cap — self-serve (Fable 5)',
    id: 'data-usage-credits-spend-cap-self-fable5',
    description: 'Monthly spend cap message for self-serve users — offers /usage-credits to manage the limit',
  },
  {
    matcher: t => t.includes('Ask your admin to turn on usage credits to continue using Fable 5'),
    name: 'Data: Usage credits limit — admin enable (Fable 5)',
    id: 'data-usage-credits-limit-admin-fable5',
    description: 'Fable 5 limit message for non-self-serve users — instructs them to ask admin to enable usage credits',
  },
  {
    matcher: t => t.includes("You've reached your Fable 5 limit. Run /usage-credits to continue"),
    name: 'Data: Usage credits limit — self-serve (Fable 5)',
    id: 'data-usage-credits-limit-self-fable5',
    description: 'Fable 5 limit message for self-serve users — offers /usage-credits to continue or /model to switch',
  },
  {
    matcher: t => t.includes('Your organization is out of usage credits. Contact your admin to add more'),
    name: 'Data: Usage credits org out',
    id: 'data-usage-credits-org-out',
    description: 'Org-level usage credit exhaustion message — instructs contact with admin to add more credits',
  },
  {
    matcher: t => t.includes('usage credit cap is reached for this period'),
    name: 'Data: Usage credits org cap reached',
    id: 'data-usage-credits-org-cap-reached',
    description: 'Org-level spend cap reached message — instructs contact with admin to raise the cap',
  },
];
